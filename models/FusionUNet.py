# -*- coding=utf-8 -*-
# ref: https://github.com/Zongyi-Lee/FusionU-Net
...
class FuseBlock(nn.Module):
    ...
    def forward(self, fp1, fp2, fp3, fp4):
        ...
        Args:
            fp1 (torch.Tensor): (B, C, H, W) 14*
            fp2 : (B, C * 2, H // 2, W // 2) 28*
            fp3: (B, C * 4, H // 4, W // 4) 56*
            fp4: (B, C * 8, H //8, W // 8) 112*
...
def reshape_downsample(x):
    ...
    # ret = ret.reshape(b, c * 4, torch.div(h,2,rounding_mode='trunc'), -1)
    ret[:, 0::4, :, :] = x[:, :, 0::2, 0::2]
    ret[:, 1::4, :, :] = x[:, :, 0::2, 1::2]
    ret[:, 2::4, :, :] = x[:, :, 1::2, 0::2]
    ret[:, 3::4, :, :] = x[:, :, 1::2, 1::2]
...
def reshape_upsample(x):
    ...
    # ret = ret.reshape(b, torch.div(c,4,rounding_mode='trunc'), h * 2, w * 2)
    ret[:, :, 0::2, 0::2] = x[:, 0::4, :, :]
    ret[:, :, 0::2, 1::2] = x[:, 1::4, :, :]
    ret[:, :, 1::2, 0::2] = x[:, 2::4, :, :]
    ret[:, :, 1::2, 1::2] = x[:, 3::4, :, :]
